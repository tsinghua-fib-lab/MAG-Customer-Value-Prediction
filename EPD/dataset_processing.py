...
class read_motif(object):
    ...
    def __iter__(self):
        ...
        if self.motif == 'triangle':
            self.path = './data/triangle/new-network/'
            self.num = 13
...
class CVPDataset(Dataset):
    ...
    def _load_data(self):
        ...
        motif_squ = 0
        data_all = np.load(os.path.join('./data', 'data_all.npy'), allow_pickle=True)
        if self.season == 'q1':
            data = data_all[:10956,:]
        elif self.season == 'q2':
            data = data_all[10956:21912,:]
        elif self.season == 'q3':
            data = data_all[21912:32868,:]
